...
export const createASpot =
 (data, { image }) =>
 async (dispatch) => {
  console.log("reducer createASpot data, { image }:", data, { image });
  let spot;
  let spotImage;
  ...
  if (response1.ok) {
   ...
   if (response2.ok) {
    spotImage = await response2.json();

    spot = { ...spot, previewImage: spotImage.imageUrl, avgRating: "New" };

    console.log("createASpot (response2.ok) spot: ", spot);
    dispatch(addASpot(spot));
    //return spot;
   } else {
    return response2;
...
const initialState = { allSpots: {}, allCurrent: {}, singleSpot: {} };
