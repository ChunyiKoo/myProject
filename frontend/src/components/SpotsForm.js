...
const SpotsForm = ({ formType }) => {
 ...
 const initialData = () => {
  ...
  setServiceFee(spots.singleSpot.serviceFee);
 };
 ...
 useEffect(() => {
  ...
  if (!serviceFee || serviceFee === null || serviceFee === 0)
   errs.push("Service fee  is required");
 ...
 return (
  <div className="new-spot-form-container">
   ...
   <form className="new-spot-form" onSubmit={handleSubmit}>
    ...
    <label className="small-text">
     <div className="where-text">
      <p className="medium-text diviser">Serve fee</p>
      <p className="small-text"></p>
     </div>
     <div className="servce-fee-money">
      &#160;$&#160;
      <input
       type="number"
       value={serviceFee}
     ...
     <p className="error-message">
      {errors.filter((err) => err.includes("Service fee"))}
     </p>
     {/* {<p className="error-message">{bErrs?.price}</p>} */}
    </label>
