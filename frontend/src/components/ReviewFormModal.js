...
function ReviewFormModal({ spotId, spotName }) {
 ...
 useEffect(() => {
  ...
  if (showErr) {
   //showErr means hasBeenSubmitted at least once
   if (errs.length === 0) setDisabled(false);
   if (run === false) setRun(true);
   setErrors(errs);
  } else {
   //hasNotBeenSubmitted before
   if (errs.length === 0) setRun(true); //can dispatch Thunk
   else setRun(false); //cannot dispatch Thunk
  }
