...
router.post("/:spotId/bookings", requireAuth, async (req, res, next) => {
 ...
 let start = new Date(startD); //correct date not one day behind
 let end = new Date(endD); //correct date not one day behind

 console.log(
  "start and today and start - Today: ",
  start,
  Today,
  start - Today
 );
 if (start - Today <= 0) {
  ...
  return res.json({
   ...
   statusCode: 400,
   errors: {
    startDate: "Ckeck-in Date cannot come before today",
   },
  });
 }
